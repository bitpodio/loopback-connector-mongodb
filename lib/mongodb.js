...
MongoDB.prototype.all = function all(model, filter, options, callback) {
  ...
  logger.info('query start',model, filter);
  filter = filter || {};
  var idName = self.idName(model);
  var query = {};
  if (filter.where) {
  ...
  function processResponse(err, cursor) {
    ...
    cursor.toArray(function(err, data) {
      logger.info('query end',model, filter);
      logger.error('query end',model, filter);
      logger.debug('query end',model, filter);
      if (self.debug) {
