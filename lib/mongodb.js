 ...
 * Module dependencies
 */
var isStackDriverEnabled = process.env.STACKDRIVER_ENABLED;
var LoggingWinston, loggingWinstonObj, winston, logger;
if(isStackDriverEnabled){
  LoggingWinston = require('@google-cloud/logging-winston').LoggingWinston;
  loggingWinstonObj = new LoggingWinston({
    prefix: 'mongo-connector-logs'
  });
...
var queryDebug = require('debug')('loopback:connector:mongodb:query');
    ...
    if (prop && prop.type === Buffer) {
      ...
      if (data[p] instanceof mongodb.Binary) {
        ...
        this.settings.enableGeoIndexing === true) {
          if (data[p].coordinates && data[p].coordinates.length === 2) {
            data[p] = {
              lat: data[p].coordinates[1],
              lng: data[p].coordinates[0],
            };
          } else {
            console.error("Invalid data point for GeoPoint field.", data[p],' id: ',data.id, ' ModelName: ', model);
            data[p] = null;
          }
    } else if(data[p] && prop && prop.type && typeof prop.type == "function" && prop.type.modelName){
      let embedModelName = prop.type.modelName;
      data[p] = self.fromDatabase(embedModelName,data[p]);
    }
  }
...
MongoDB.prototype.toDatabase = function(model, data) {
  ...
  for (var p in props) {
    ...
    // Check for if a field is embeds one . If yes, checking if any field in that model is of geo field @madhav mohtta
    // Fix against workpay bug no 1540
    if(data[p] && prop && prop.type && typeof prop.type == "function" && prop.type.modelName){
      let embedModelName = prop.type.modelName;
      data[p] = self.toDatabase(embedModelName,data[p]);
    }
  }
...
MongoDB.prototype.execute = function(model, command) {
  ...
  var queryLogCallback = function(model, query = {}, callback) {
    var startTs = Date.now();
      return function(err, result) {
        ...
        queryDebug('TOTAL QUERY TIME %s (ms)', endTs - startTs);
        if (queryDebug.enabled || isStackDriverEnabled) {
          let queryFormat = query.slice(0, query.length -1);
          try {
            queryFormat = JSON.stringify(queryFormat);
          } catch (error) {
            console.error(queryFormat);
          }
          queryDebug('Model: %s QUERY %s', model, queryFormat);
          if(isStackDriverEnabled)
          logger.debug(`TOTAL QUERY TIME ${endTs - startTs} (ms), Model: ${model}, QUERY ${queryFormat}`);
        }
        callback(err, result);
      };
  }
  this.notifyObserversAround('execute', context, function(context, done) {
    args[args.length - 1] = queryLogCallback(model, args, done);
 ...
 * @param {Function} [callback] The callback function
 */
MongoDB.prototype.updateOrCreate = function updateOrCreate(model, filter, data, options, callback) {
  var self = this;
  if (self.debug) {
  ...
  if (filter && filter.where) {
    filter = self.buildWhere(model, filter.where);
  } else {
    filter =   {
      _id: oid,
    };
  ...
  if (self.debug) {
    debug('updateOrCreate > filter: ', filter);
  }
  this.execute(model, 'findAndModify', filter, [
    ['_id', 'asc'],
...
//query.where = _.merge(query.where, {})
//_.chunk(_.split(near.key,"."))
_.set(query.where,near.key,nearqueryObject);
  ...
  props.forEach(function(p) {
    if (near[p]) {
      let distanceQuery = _.clone(near.key);
      distanceQuery.push("near");
      distanceQuery.push(`$${p}`);
      _.set(query.where,distanceQuery,convertToMeters(near[p], unit));
      //_.set(query.where,`${near.key}.near.$${p}`,convertToMeters(near[p], unit));
    ...
    cursor.toArray(function(err, data) {
      ...
      var objs = data.map(function(o) {
        ...
        try {
          ...
          console.error('Error while parsing data object (after querying) from db. Model: ', model, ' data: ', data.id, error);
          return null;
        }
      });
      objs = objs.filter(Boolean); // remove nulls
